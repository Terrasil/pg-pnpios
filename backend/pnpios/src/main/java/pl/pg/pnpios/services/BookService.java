...
public class BookService {
    ...
    public BookSearchResponseDTO searchBooks(String q, int page, int size, String currency) {
        ...
        List<BookSearchItemDTO> allBooks = List.of(
            new BookSearchItemDTO(
                ...
                3,
                new PriceRangeDTO(
                    convert(BigDecimal.valueOf(39.99), "PLN", targetCurrency),
                    convert(BigDecimal.valueOf(59.99), "PLN", targetCurrency),
                    targetCurrency
                )
            ),
            new BookSearchItemDTO(
                ...
                3,
                new PriceRangeDTO(
                    convert(BigDecimal.valueOf(29.99), "PLN", targetCurrency),
                    convert(BigDecimal.valueOf(49.99), "PLN", targetCurrency),
                    targetCurrency
                )
            ),
            new BookSearchItemDTO(
                ...
                3,
                new PriceRangeDTO(
                    convert(BigDecimal.valueOf(79.99), "PLN", targetCurrency),
                    convert(BigDecimal.valueOf(129.99), "PLN", targetCurrency),
                    targetCurrency
                )
            ),
            new BookSearchItemDTO(
                ...
                2,
                new PriceRangeDTO(
                    convert(BigDecimal.valueOf(24.99), "PLN", targetCurrency),
                    convert(BigDecimal.valueOf(39.99), "PLN", targetCurrency),
                    targetCurrency
                )
            )
        ...
        List<BookSearchItemDTO> filtered = allBooks.stream()
            .filter(book ->
                query.isBlank()
                    || normalize(book.title()).contains(query)
                    || normalize(book.isbn13()).contains(query)
        ...
        return new BookSearchResponseDTO(
            ...
            size,
            filtered.size(),
            calculateTotalPages(filtered.size(), size),
    ...
    public BookDetailsDTO getBookDetails(String bookId, String currency) {
        ...
        return switch (bookId) {
            case "book_1" -> new BookDetailsDTO(
                "book_1",
                "The Fellowship of the Ring",
                null,
                "Pierwsza część trylogii Władca Pierścieni.",
                List.of(new AuthorShortDTO("author_1", "J. R. R. Tolkien")),
                null,
                "George Allen & Unwin",
                1954,
                "en",
                ...
                "0261103571",
                "9780261103573",
                getBookOffers("book_1", targetCurrency, OfferSortType.PRICE_ASC, null)
            );
            case "book_2" -> new BookDetailsDTO(
                "book_2",
                "Harry Potter and the Philosopher's Stone",
                null,
                "Pierwsza część serii o Harrym Potterze.",
                List.of(new AuthorShortDTO("author_2", "J. K. Rowling")),
                null,
                "Bloomsbury",
                1997,
                "en",
                ...
                "0747532699",
                "9780747532699",
                getBookOffers("book_2", targetCurrency, OfferSortType.PRICE_ASC, null)
            );
            case "book_3" -> new BookDetailsDTO(
                "book_3",
                "Clean Code",
                "A Handbook of Agile Software Craftsmanship",
                "Książka o dobrych praktykach programistycznych.",
                List.of(new AuthorShortDTO("author_3", "Robert C. Martin")),
                null,
                "Prentice Hall",
                2008,
                "en",
                ...
                "0132350882",
                "9780132350884",
                getBookOffers("book_3", targetCurrency, OfferSortType.PRICE_ASC, null)
            );
            case "book_4" -> new BookDetailsDTO(
                "book_4",
                "Ostatnie życzenie",
                null,
                "Zbiór opowiadań otwierający cykl o Wiedźminie.",
                List.of(new AuthorShortDTO("author_4", "Andrzej Sapkowski")),
                null,
                "SuperNOWA",
                1993,
                "pl",
                ...
                "8375780637",
                "9788375780635",
                getBookOffers("book_4", targetCurrency, OfferSortType.PRICE_ASC, null)
            );
            default -> new BookDetailsDTO(
                bookId,
                "Nieznana książka",
                null,
                "Brak opisu.",
    ...
    public List<OfferDTO> getBookOffers(String bookId, String currency, OfferSortType sort, String source) {
        ...
        if (source != null && !source.isBlank()) {
            offers = offers.stream()
                .filter(offer -> offer.source().equalsIgnoreCase(source))
                .collect(Collectors.toList());
        }
    ...
    private OfferDTO createOffer(
        ...
        MoneyDTO convertedPrice = new MoneyDTO(
            convert(BigDecimal.valueOf(originalAmount), originalCurrency, targetCurrency),
            targetCurrency
        );
