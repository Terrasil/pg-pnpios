...
public class AuthorService {
    ...
    public AuthorDetailsDTO getAuthorDetails(String authorId) {
        return switch (authorId) {
            case "author_1" -> new AuthorDetailsDTO(
                "author_1",
                "J. R. R. Tolkien",
                1892,
                1973,
                "Brytyjski pisarz i filolog, autor literatury fantasy, najbardziej znany z Władcy Pierścieni.",
                null,
                findBooksByAuthorName("J. R. R. Tolkien", "PLN")
            );
            case "author_2" -> new AuthorDetailsDTO(
                "author_2",
                "J. K. Rowling",
                1965,
                null,
                "Brytyjska pisarka, autorka serii o Harrym Potterze.",
                null,
                findBooksByAuthorName("J. K. Rowling", "PLN")
            );
            case "author_3" -> new AuthorDetailsDTO(
                "author_3",
                "Robert C. Martin",
                1952,
                null,
                "Programista i autor książek o czystym kodzie i architekturze oprogramowania.",
                null,
                findBooksByAuthorName("Robert C. Martin", "PLN")
            );
            case "author_4" -> new AuthorDetailsDTO(
                "author_4",
                "Andrzej Sapkowski",
                1948,
                null,
                "Polski pisarz fantasy, autor cyklu o Wiedźminie.",
                null,
                findBooksByAuthorName("Andrzej Sapkowski", "PLN")
            );
            default -> new AuthorDetailsDTO(
                authorId,
                "Nieznany autor",
                null,
                null,
    ...
    public BookSearchResponseDTO getAuthorBooks(String authorId, int page, int size, String currency) {
        ...
        return new BookSearchResponseDTO(
            ...
            size,
            books.size(),
            calculateTotalPages(books.size(), size),
            paginate(books, page, size)
        );
    }

    private List<BookSearchItemDTO> findBooksByAuthorName(String authorName, String currency) {
        return bookService.searchBooks("", 0, 100, currency)
            .items()
            .stream()
